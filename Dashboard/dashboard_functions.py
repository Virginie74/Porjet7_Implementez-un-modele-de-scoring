...
def preprocessor(data_customer):
    ...
    for col in X_train_cat:
        ...
        if (X_train_cat.loc[:, col].dtype == 'object'
            ...
            X_train_cat.loc[:, col] = encoder.transform(X_train_cat.loc[:, col])
...
def plot_radar(df_MM, df_mean_repaid, df_mean_default):
    ...
    values=radar_df.loc[0].drop("Groupe").values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, color='#929591', linewidth=1, linestyle='solid', label="Client")
    ax.fill(angles, values, color='#929591', alpha=0.4)

    # Ind2
    values=radar_df.loc[1].drop("Groupe").values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, color='#40E0D0', linewidth=1, linestyle='solid', label="Moyenne crédit accordé")
    ax.fill(angles, values, color='#40E0D0', alpha=0.1)

    # Ind2
    values=radar_df.loc[2].drop("Groupe").values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, color='#FF6347', linewidth=1, linestyle='solid', label="Moyenne crédit refusé")
...
def boxplot_for_num_feature(df, feature, data_customer):
    ...
    ax = sns.boxplot(data=data, x="TARGET", y=feature, palette=["darkturquoise", "tomato"])
    ax = sns.stripplot(data=data, x="TARGET", y=feature, alpha=0.6)
    ax.hlines(y=data_customer[feature].values, xmin=-1, xmax=2, color='#ff3300', linestyle='--', linewidth=3, label='Valeur du client')
